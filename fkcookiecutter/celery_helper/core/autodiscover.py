...
def autodiscover_task_imports(packages=None, related_name='tasks', task_prefix='task_'):
    ...
    """
    task_imports = []
    project_name = os.environ.get("APP_NAME")
    installed_apps = get_installed_apps()

    packages = packages() if callable(packages) else packages or ()
    packages_or_apps = list(packages) + installed_apps
    module_package = __package__.split('.')[0] + '.apps'

    for name in packages_or_apps:
        package = importlib.import_module(name, package=module_package)
        ...
        for module_info in pkgutil.iter_modules(package.__path__, package.__name__ + "."):
            ...
            is_tasks_pkg = module_info.ispkg  # Check task package
